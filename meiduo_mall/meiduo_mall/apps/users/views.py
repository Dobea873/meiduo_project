...
class UserBrowseHistory(LoginRequiredJSONMixin, View):
    ...
    def get(self, request):
        ...
        redis_conn = get_redis_connection('history')
        # 取出列表数据（核心代码）
        sku_ids = redis_conn.lrange('history_%s' % user.id, 0 , -1)  # or (0, 4)
...
class AddressView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # 使用当前登录用户is_deleted=False作为条件查询地址数据
        addresses = Address.objects.filter(user=login_user, is_deleted=False)

        # 将用户地址模型列表转字典列表：因为JsonResponse和Vue.js不认识模型类型，只有Django和jinja2模版引擎认识
        address_list = []
...
class EmailView(LoginRequiredJSONMixin, View):
    ...
    def put(self, request):
        ...
        send_verify_email.delay(email, verify_url)  # 一定记得调用delay
