        ...
        with transaction.atomic():
            ...
            try:
                ...
                for sku_id in sku_ids:
                    ...
                    while True:
                        # 读取购物车商品信息
                        sku = SKU.objects.get(id=sku_id)  # 查询商品和库存信息时，不能出现缓存，所以没用filter（id_in=sku_ids）
                        ...
                        # 如果在更新数据时，原始数据变化了，放回0；表示有资源抢夺
                        if result == 0:
                            # 库存 10 要买1 但是在下单时，有资源抢夺，被买走1，剩下9，如果库存依然满足继续下单，直到库存不足
