...
# 创建日志输出器
logger = logging.getLogger('djaongo')
...
class QQAuthUserView(View):
    ...
    def get(self,request):
        ...
        try:
            ...
            # openid已绑定美多商场用户:oauth_user.user表示从QQ登录模型类对象中找到对应的用户模型类对象
    ...
    def post(self, request):
        ...
        try:
            ...
            if not user.check_password(password):
                return render(request, 'oauth_callback.html', {'acount_errmsg':'账号或密码错误'})

        # 将新建用户，已存在用户绑定到openid
        # oauth_qq_user = OAuthQQUser(user=user, openid=poenid)
        # oauth_qq_user.save()
        try:
