...
# 分页器(有100万字，需要制作成一本书，先规定每页多少文字，然后得出一共多少页)
...
class DetailVisitView(View):
    ...
    def post(self, request, category_id):
        ...
        # 将当天的时间字符串转成时间对象datetime,为了跟data字段的类型匹配
...
class HotGoodsView(View):
    ...
    def get(self, request, category_id):
        # 要查询指定分类的SKU信息，而且必须是上架的状态，然后按照销量由高到低排序，最后切片取出前两位
...
#         """查询并渲染商品列表页"""
#
#         # 校验参数category_id的范围: 111111111111111111111111
#         try:
#             # 三级类别
#             category = GoodsCategory.objects.get(id=category_id)
#         except GoodsCategory.DoesNotExist:
...
#         else:  # 只要不是'price'和'-sales'其他的所有情况都归为'defult'
#             sort = 'default' # 当出现？sort=itcast 也把sort设置为'default'
...
#
#         # 分页和排序查询：category查询sku，一查多，一方的模型对象，关联字段.all/filter
